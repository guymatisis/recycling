...
$action_str = "admin_add_delete_supers.php?current_user=".$current_user;
   ...
   $fields_incomplete = False;
    ...
    if (!$fields_incomplete)
    {
       ...
       $insert_Query = "INSERT INTO `Employees`
        (First_Name, Last_Name,Phone_Number,DOB,Address_ID,Password,Email,User_Name,Muni_ID) 
        VALUES 
        ('$data[1]','$data[2]','$data[12]',STR_TO_DATE('$data[3]', '%Y-%m-%d'),LAST_INSERT_ID(),'$data[4]','$data[6]','$data[14]', $data[11])";
        try{
            $insert_Result = mysqli_query($conn, $insert_Query);
